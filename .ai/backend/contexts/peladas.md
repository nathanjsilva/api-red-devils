...
- `division`: obrigatório na criação (`required|in:quinta,sabado`), `sometimes|in:quinta,sabado` na edição. Representa a "divisão"/liga da pelada — hoje só existem duas: peladas de quinta-feira e de sábado. **Regra de consistência**: `withValidator()` em `StorePeladaRequest`/`UpdatePeladaRequest` rejeita a combinação se o dia da semana de `date` não bater com a divisão (quinta → quinta-feira, sabado → sábado); no `update`, quando só um dos dois campos é enviado, o outro é resolvido a partir do registro já salvo antes de checar. Coluna adicionada em `database/migrations/2026_07_13_220826_add_division_to_peladas_table.php`, com backfill automático das peladas antigas pelo dia da semana real da `date` (default `'quinta'` cobre o caso comum; qualquer pelada existente num sábado é migrada para `'sabado'` via `UPDATE ... WHERE DAYOFWEEK(date) = 7`).
